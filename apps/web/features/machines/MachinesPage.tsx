...
export function MachinesPage() {
  ...
  async function saveMachine(values: Parameters<typeof machines.saveMachine>[0]) {
    ...
    setFormOpen(false);
    setFeedback(editingMachine ? 'Machine details updated.' : 'Machine added to the register.');
  }
  ...
  async function confirmRetire() {
    ...
    setFeedback(`${retiringMachine.name} was moved to retired.`);
  }
    ...
    <AnimatedPage>
      <div className="mx-auto flex min-h-screen max-w-[1500px]">
        ...
        <div className="min-w-0 flex-1">
          <header className="flex items-center justify-between border-b border-[#dfe4df] px-5 py-5 sm:px-8 lg:px-10">
            ...
            <AppButton onClick={openCreate}>Add machine</AppButton>
          </header>
          <main className="px-5 py-7 sm:px-8 sm:py-9 lg:px-10 lg:py-12">
            <AnimatedSection className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
              <div>
                ...
                <p className="mb-0 mt-3 max-w-xl text-sm leading-6 text-[#68736f]">
                  Keep the equipment register accurate so maintenance work starts with the right
                  context.
                </p>
              </div>
              <div className="text-left sm:text-right">
                ...
                <p className="m-0 text-xs font-semibold text-[#68736f]">registered machines</p>
              </div>
            </AnimatedSection>
            {machines.error ? (
              <div
              ...
              >
                <div>
                  <p className="m-0 text-sm font-bold text-[#8e2f28]">Register unavailable</p>
            ...
            <AppPanel className="mb-5" title="Filter register" eyebrow="Find a production asset">
              <MachineFilters
            ...
            {pageCount > 1 ? (
              <div className="mt-5 flex items-center justify-end gap-3">
                ...
                <span className="text-xs font-bold text-[#68736f]">
                  Page {machines.filters.page} of {pageCount}
                </span>
                <AppButton
      ...
      >
        <p className="mb-0 text-sm leading-6 text-[#68736f]">
          {retiringMachine
            ? `${retiringMachine.name} will remain in the register as retired and will not be deleted.`
            : ''}
        </p>
      </AppModal>
    </AnimatedPage>
