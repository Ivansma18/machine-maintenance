...
- Comandos actuales: `npm install`, `npm run dev:web`, `npm run build:web`, `npm run start:api`, `npm run build:api`, `npm run typecheck:api`, `npm run prisma:validate`, `npm run prisma:generate`, `npm run prisma:migrate:dev`, `npm run prisma:migrate:deploy`, `npm run prisma:seed`.
